"use client";

import { useEffect, useState } from "react";
import { toast } from "sonner";

import { useUnsavedChanges } from "@/components/admin/use-unsaved-changes";
import { Button } from "@/components/ui/button";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

export const entityFormTabs = ["content", "seo", "faqs", "media", "settings"] as const;
export type EntityFormTab = (typeof entityFormTabs)[number];

export type EntityFormSlots = {
  content: React.ReactNode;
  seo?: React.ReactNode;
  faqs?: React.ReactNode;
  media?: React.ReactNode;
  settings?: React.ReactNode;
};

export type EntityFormProps = {
  title: string;
  description?: string;
  isDirty: boolean;
  slots: EntityFormSlots;
  defaultTab?: EntityFormTab;
  onSaveDraft: () => Promise<{ ok: boolean; error?: string }>;
  onPublish?: () => Promise<{ ok: boolean; error?: string }>;
  autosaveMs?: number;
};

const TAB_LABELS: Record<EntityFormTab, string> = {
  content: "Content",
  seo: "SEO",
  faqs: "FAQs",
  media: "Media",
  settings: "Settings",
};

export async function runOptimistic(
  work: () => Promise<{ ok: boolean; error?: string }>,
  messages: { loading: string; success: string },
): Promise<boolean> {
  const id = toast.loading(messages.loading);
  const result = await work();
  if (result.ok) {
    toast.success(messages.success, { id });
    return true;
  }
  toast.error(result.error ?? "Something went wrong.", { id });
  return false;
}

export function EntityForm({
  title,
  description,
  isDirty,
  slots,
  defaultTab = "content",
  onSaveDraft,
  onPublish,
  autosaveMs = 8000,
}: EntityFormProps) {
  const [tab, setTab] = useState<EntityFormTab>(defaultTab);
  const [saving, setSaving] = useState(false);
  useUnsavedChanges(isDirty);

  useEffect(() => {
    if (!isDirty) {
      return;
    }
    const timer = window.setTimeout(() => {
      void runOptimistic(onSaveDraft, {
        loading: "Saving draft…",
        success: "Draft saved",
      });
    }, autosaveMs);
    return () => window.clearTimeout(timer);
  }, [autosaveMs, isDirty, onSaveDraft]);

  const visibleTabs = entityFormTabs.filter((id) => id === "content" || slots[id]);

  async function handleDraft() {
    setSaving(true);
    await runOptimistic(onSaveDraft, { loading: "Saving draft…", success: "Draft saved" });
    setSaving(false);
  }

  async function handlePublish() {
    if (!onPublish) {
      return;
    }
    setSaving(true);
    await runOptimistic(onPublish, { loading: "Publishing…", success: "Published" });
    setSaving(false);
  }

  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">{title}</h1>
          {description ? <p className="text-sm text-muted-foreground">{description}</p> : null}
        </div>
        <div className="flex gap-2">
          <Button type="button" variant="outline" disabled={saving || !isDirty} onClick={() => void handleDraft()}>
            Save draft
          </Button>
          {onPublish ? (
            <Button type="button" disabled={saving} onClick={() => void handlePublish()}>
              Publish
            </Button>
          ) : null}
        </div>
      </div>
      <Tabs value={tab} onValueChange={(value) => setTab(value as EntityFormTab)}>
        <TabsList variant="line" className="w-full justify-start overflow-x-auto">
          {visibleTabs.map((id) => (
            <TabsTrigger key={id} value={id}>
              {TAB_LABELS[id]}
            </TabsTrigger>
          ))}
        </TabsList>
        {visibleTabs.map((id) => (
          <TabsContent key={id} value={id} className="pt-4">
            {slots[id]}
          </TabsContent>
        ))}
      </Tabs>
    </div>
  );
}
